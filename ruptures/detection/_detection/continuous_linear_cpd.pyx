...
cpdef continuous_linear_pelt(double[:] signal, double beta, int min_size):
    """Optimal linear spline regression with fixed number of knots.
    ...
    cdef double[::1] signal_arr = np.ascontiguousarray(signal)
    try:
        clin.continuous_linear_pelt_c(&signal_arr[0], n_samples, beta, min_size, &path_matrix_flat[0])
    except:
        print("An exception occurred.")
