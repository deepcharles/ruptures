...
def pw_gaussian_mixture(n_samples=200, n_features=1, n_bkps=3, noise_std=None,n_components=2, ratio=[0.8,0.2], mean_range=(1,10)):
    ...
    for subarray in np.split(indices, breakpoints):
        if subarray.size > 0:
            ...
            gaussian_mixture_segment = get_gaussian_mixture_segment(n_features, ratio, means, standard_deviations, subarray.size)
